...
# -*- coding: utf-8 -*-
"""
Video Tracker Test - V3: Full Features (Shape + Depth + HSV = 1584-dim)
Test tracking trên video với đầy đủ features.

Usage:
    python3 test_v3_full.py --video /path/to/video.mp4
"""
...
def hsv_histogram(roi_bgr, bins=16, v_weight=0.5, normalize_brightness=True):
    """Trích xuất HSV histogram - 48-dim"""
...
def extract_depth_feature(box, depth_img, target_size=(16, 16)):
    """Trích xuất depth feature 256-dim"""
...
def extract_full_feature(frame, box, depth_img, ort_sess, color_weight=0.3, depth_weight=0.1):
    """V3: MobileNetV2 + Depth + HSV - 1584-dim"""
    roi_padded = body_arr_preserve_aspect_ratio(frame, box)
    if roi_padded is None:
        return None

    # 1. Shape feature (MobileNetV2) - 1280-dim
    ...
    hsv_feat = hsv_histogram(roi_padded, bins=16, v_weight=0.6)
    
    # 3. Depth feature - 256-dim
    depth_feat = extract_depth_feature(box, depth_img)
    
    # Combine with weights
    shape_weighted = emb * (1.0 - color_weight)
    hsv_weighted = hsv_feat * color_weight
    depth_weighted = depth_feat * depth_weight
    
    # 1280 + 48 + 256 = 1584
    feat = np.concatenate([shape_weighted, hsv_weighted, depth_weighted])
...
class VideoTracker:
    ...
    def enroll(self, frame, boxes):
        ...
        feat = extract_full_feature(frame, box, self.depth_img, self.mb2_sess)
        if feat is not None:
    ...
    def get_summary(self, total_time):
        ...
        return {
            'version': 'V3 (Full: Shape + Depth + HSV)',
...
# ========== Main ==========
def main():
    parser = argparse.ArgumentParser(description='Test V3: Full Features')
    ...
    parser.add_argument('--output', type=str, default='results_v3_full.csv')
    parser.add_argument('--show', action='store_true', help='Show video')
    args = parser.parse_args()
    
    print(f"=== V3: Full Features (Shape + Depth + HSV = 1584-dim) ===")
    ...
    while True:
        ...
        if args.show:
            ...
            cv2.imshow('V3: Full Features', frame)
    ...
    print(f"\n{'='*50}")
    print(f"=== RESULTS: V3 (Full: Shape + Depth + HSV) ===")
    print(f"{'='*50}")
