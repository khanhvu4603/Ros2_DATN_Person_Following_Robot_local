...
# -*- coding: utf-8 -*-
"""
Full Features Tracker - Uses all available features.

Feature dimension: 1280-D (MobileNet) + 48-D (HSV) + 256-D (Depth) = 1584-D
Current implementation from person_detector.py.
Expected FPS: 6-7 | Expected Accuracy: ~85%
"""
...
class FullFeaturesTracker(BaseTracker):
    """
    Tracker using all features: MobileNetV2 + HSV + Depth (1584-D total).
    Most accurate but slowest variant.
    """
    ...
    def process_frame(
        ...
        """
        Process frame using all features (MobileNetV2 + HSV + Depth).
        ...
        if self.target_feature is None:
            ...
            self.target_feature = self._extract_full_feature(
        ...
        # Find best match by full feature similarity
        best_box, best_score = None, 0.0
        
        for box in detections:
            # Extract full feature
            feat = self._extract_full_feature(rgb_frame, box, depth_frame)
            if feat is None:
                continue
        ...
        if best_score >= self.similarity_threshold:
            ...
            new_feat = self._extract_full_feature(rgb_frame, best_box, depth_frame)
    ...
    def _extract_full_feature(
        ...
        """
        Extract all features: MobileNetV2 (1280-D) + HSV (48-D) + Depth (256-D) = 1584-D.
